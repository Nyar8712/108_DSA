...
#     這邊就開始做排序了，根據我在老師提供的程式碼和閱讀書得到的了解，基準點可以任意挑選不受限制，所以我先import random套件進來，做隨機選資料值。
#     我打算用陣列來做，因此先建了三個list，然後把不需要排序的可能先找出來，也就是測值僅有一筆或根本沒資料時，這時就直接回傳測值內容就好了。

#     用判斷式來進行分群，當測值小於基準時會被加入left中，大於則會加入right，相等的值加入mid。
#     如此一來，在迴圈進行完後，會得到三組群，如同上面partition第(3)所說的"小於基準值的群"+"等於基準值的群"+"大於基準值的群"
#     接下來只要再對left和right反覆做相同的事情，不斷分群然後排序，直到全部排序完成，在將這三個list內的值回傳，就完成快速排序了。
